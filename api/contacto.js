...
module.exports=async(req,res)=>{
 if(req.method==="GET"){
   ...
   return res.status(200).json({ok:true,service:"RazerTech contacto API",environment:Object.fromEntries(keys.map(k=>[k,Boolean(process.env[k])]))});
 }
 ...
 try{
   ...
   await transporter.sendMail({
     from:`"RazerTech" <${process.env.SMTP_USER}>`,
     to:process.env.CONTACT_EMAIL||process.env.SMTP_USER,
     replyTo:email,
     subject:"Nueva consulta RazerTech - madridtechsupport.com.es",
     text:`Nombre: ${nombre}\nTeléfono: ${telefono}\nEmail: ${email}\nEquipo: ${equipo}\n\nConsulta:\n${mensaje}`
   });
   return res.status(200).json({ok:true});
 }catch(error){
   console.error("RazerTech SMTP error",{message:error?.message,code:error?.code,response:error?.response});
